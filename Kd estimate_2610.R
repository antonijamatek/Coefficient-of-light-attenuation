...
#6 Estimating K for each day using nls (least squares method)
set.seed(1)
...
for (i in 1:length(K1)) #loop through each day's K estimate and plot the light decay with depth 
{
  ...
  lines(Iest, depth1, type="b", col=color_palette[i], lwd=1)
}
for (i in 1:length(n1)) 
{ subset_data1 <- l1[l1$Date == n1[i],] #subset the data for the current date
lines(subset_data1$`Light intensity(Lux)`, depth1, lty=2, lwd=2, col=color_palette[i])  #plot the measured light intensity data for each date n
}
